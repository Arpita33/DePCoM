...
def train(hidden_sz, layers, dropout, batch_size, learning_rate, num_epochs, exp_no, k_val):
	...
	for epoch in range(EPOCHS):
		...
		if v_auc>best_valid_auc:
			...
			model_folder = "./Models/k_"+str(k_val)+"/"
...
def test(hidden_sz, layers, dropout, batch_size, learning_rate, num_epochs, exp_no, k_val):
    #perform evaluation on test set here
    model_path = "./Models/k_2/model_1_3"
    model = GRU_V3(batch_size, hidden_sz, layers, dropout)
    ...
    test_loss, test_acc, test_pr, test_re, test_f1, test_auc =evaluate_model(model,test_dataloader,batch_size)
...
def main():
    ...
    #test(args.hidden_dims, args.num_layers, args.dropout, args.batch_size, args.lr, args.epochs, args.exp_no, args.k)
