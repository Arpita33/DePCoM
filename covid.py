...
def get_data_min_max(records):
	...
	for b, (record_id, tt, vals, mask, labels) in enumerate(records):
		...
		for i in range(n_features):
			...
			#print("In get data min max:",non_missing_vals)
...
class Covid(object):
    ...
    def __init__(self, type_,k_val,n_samples = None, device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")):
       ...
       'Peripheral vascular disease', 'Renal disease', 'Rheumatic disease']
        ...
        self.processed_folder = "./k_folds/processed/k_"+str(k_val)
        if self.Type == "Train":
            #self.process("../../Cov_Data_Verified_Training",self.processed_folder,Train=True)
            self.process("./k_folds/raw/k_"+str(k_val)+"/Train/", "train", k_val)
            data_file = self.training_file
        elif self.Type == "Valid":
            self.process("./k_folds/raw/k_"+str(k_val)+"/Validation/","validation",k_val)
            data_file = self.validation_file
        elif self.Type == "Test":
            #self.process("../../Cov_Data_Verified_Testing",self.processed_folder,Train=False)
            self.process("./k_folds/raw/k_"+str(k_val)+"/Test/","test",k_val)
    ...
    def validation_file(self):
        return 'Covid_Processed_validation.pt'    
